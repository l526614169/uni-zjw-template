<template>
	<view class="u-field" :class="{'u-border-top': borderTop, 'u-border-bottom': borderBottom }" :style="customStyle">
		<view class="u-field-inner" :class="[type == 'textarea' ? 'u-textarea-inner' : '', 'u-label-postion-' + labelPosition]">
			...
			<view class="fild-body">
				...
				<u-icon v-if="rightIcon" @click="rightIconClick" :name="rightIcon" :color="rightColor" :custom-prefix="iconPrefix" :style="[rightIconStyle]" size="26" class="u-arror-right" />
			</view>
		</view>
		<view v-if="errorMessage !== false && errorMessage != ''" class="u-error-message" :style="{
			paddingLeft: labelWidth + 'rpx',fontSize: labelSize + 'rpx'
		}">{{ errorMessage }}</view>
	</view>
</template>
